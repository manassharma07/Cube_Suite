...
st.write('### Expoenentiate CUBE file (raise the CUBE file to a certain power)')
...
if uploaded_file is not None:
    ...
    power = st.number_input('nx', 1, 10, 1, 1)

    raise_button = st.button("Exponentiate CUBE File")

    if raise_button:
        result = subprocess.run(["cube_tools", "-p", str(power), filepath], capture_output=True, text=True)
        # st.write("#### " + str(result.stdout))
        if result.stderr:
            st.write("Error:", result.stderr)
        else:
            exponentiated_file_path = f"square.cube"
